...
# train_echoes2, train_y2, test_echoes2, test_y2, dataset_name2, n_res, len_series, IS, SR, SP=train_echoes1, train_y1, test_echoes1, test_y1, dataset_name1, n_res, len_series, IS, SR, SP
# train_echoes3, train_y3, test_echoes3, test_y3, dataset_name3, n_res, len_series, IS, SR, SP=train_echoes1, train_y1, test_echoes1, test_y1, dataset_name1, n_res, len_series, IS, SR, SP
print('Transfering label...')

LLwrite(train_echoes1[0])
LLwrite(train_echoes1[1])
print(train_echoes1[1])
print(train_echoes1[0])
print(train_echoes1[1])
...
test_data1 = np.reshape(test_echoes1,(nb_sample_test1,len_series, n_res))
test_data2 = np.reshape(test_echoes2,(nb_sample_test2, len_series, n_res))
test_data3 = np.reshape(test_echoes3,(nb_sample_test3, len_series, n_res))
...
train_data1 = np.zeros((nb_sample_train1, len_series, n_res))
train_data2 = np.zeros((nb_sample_train2, len_series, n_res))
train_data3 = np.zeros((nb_sample_train3,len_series, n_res))
...
for m in range(nb_sample_train1):
    ...
    train_data1[m,:,:] = train_echoes1[L_train1[m],:,:]
    train_label1[m,:] = train_y1[L_train1[m],:]
    train_data2[m,:,:] = train_echoes2[L_train1[m],:,:]
    train_label2[m,:] = train_y2[L_train1[m],:]
    train_data3[m, :, :] = train_echoes3[L_train1[m], :, :]
    train_label3[m, :] = train_y3[L_train1[m], :]

# Data_train=np.array([train_data1,train_data2,train_data3])
# print(Data_train.shape())
Data_train=np.array([train_data1,train_data2])
Data_test=np.array([test_data1,test_data2])
Data_train=np.reshape(Data_train,(nb_sample_train1,len_series, n_res,2))
Data_test=np.reshape(Data_test,(nb_sample_test1,len_series, n_res,2))

Data_train=RUN_SE(Data_train)
...
input_shape = (len_series, n_res,2)
...
for j in range(len(nb_row)):
	...
	conv = Conv2D(nb_filter, (nb_row[j], nb_col), kernel_initializer = kernel_initializer, activation = activation, padding = 'same', strides = strides, data_format = data_format)(input)
	conv = GlobalMaxPooling2D(data_format = data_format)(conv)
	convs.append(conv)

body_feature1 = concatenate(convs,name='concat_layer1')
...
body_feature = Dropout(0.25)(body_feature1)
